...
const strengthBarSegmentVariants = cva("h-full rounded-full transition-all duration-300 ease-in-out", {
  variants: {
    strength: {
...
export function PasswordStrengthMeter({
  ...
  return (
    (<div className={cn("space-y-2", className)} {...props}>
      ...
      <div className={cn(strengthMeterVariants({ size, animated }), meterClassName)}>
        {Array.from({ length: segments }).map((_, i) => (
          <div
            key={i}
            className={cn(strengthBarSegmentVariants({
          ...
          <ul className="space-y-1.5">
            {requirements.map((requirement, index) => {
              ...
              return (
                (<li
                  key={index}
                  className={cn("flex items-center gap-2 text-sm", passed
